...
import { LayoutResult, LayoutNode, LayoutPoint } from '../../types/layout';
...
export const GraphCanvas: React.FC<GraphCanvasProps> = ({
  ...
  useEffect(() => {
    fitView();
  }, [layout.bounds.width, layout.bounds.height, fitView]);
              ...
              return (
                ...
                >
                  ...
                  <path
                    ...
                    strokeDasharray={
                      edge.kind === 'render' || edge.kind === 'dependency' ? '4 3' : undefined
                    }
            ...
            {nodes.map((node) => {
              ...
              if (runtimeState && runtimeState.value !== undefined) {
                if (typeof runtimeState.value === 'object' && runtimeState.value !== null) {
                  valPreview = Array.isArray(runtimeState.value)
                    ? `[${runtimeState.value.length} items]`
                    : `{...}`;
                } else {
              ...
              return (
                ...
                >
                  ...
                  {valPreview !== null ? (
                    ...
                    >
                      {node.source?.file
                        ? node.source.file.split('/').slice(-1)[0] + `:${node.source.line}`
                        : node.id}
                    </text>
                  )}
                </g>
